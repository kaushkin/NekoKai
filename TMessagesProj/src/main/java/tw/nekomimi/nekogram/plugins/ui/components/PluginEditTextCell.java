...
public class PluginEditTextCell extends EditTextCell {
    ...
    private void performSave(EditTextSetting editTextSetting, String value, String pId, String key) {
        if (editTextSetting.maxLength > 0 && value.length() > editTextSetting.maxLength) {
            PluginsController.getInstance().setPluginSetting(pId, key, value.substring(0, editTextSetting.maxLength));
        } else {
            PluginsController.getInstance().setPluginSetting(pId, key, value);
        }
        triggerOnChange(editTextSetting.onChangeCallback, pId, value);
    }
    ...
    private void triggerOnChange(final PyObject pyObject, final String pId, final Object obj) {
        Utilities.pluginsQueue.postRunnable(() -> performTriggerOnChange(pyObject, obj, pId));
    }

    private void performTriggerOnChange(PyObject pyObject, Object obj, String pId) {
        if (pyObject != null) {
            try {
                pyObject.call(obj);
            } catch (Exception e) {
                FileLog.e("Error executing on_change callback for " + this.pluginId + "/" + pId, e);
            }
        }
    }
